...
colFun <- function(rang, id, category, data, rank_col = T, top = c(1), flop = c(1),
	...
	df[, 'Color'] <- ifelse(df[, category] %in% nam_blue, balCol('hbl'), col_neutral)
	df[, 'Color'] <- ifelse(df[, category] %in% nam_orange, balCol('ora'), df[, 'Color'])
	
	if (rank_col == T){
		# Green color for top profiles
		
		ifelse(df[, rang] %in% top, 1, 0)
		df[, 'Color'] <- ifelse(df[, rang] %in% top, rgb(0, 100, 0, alpha = 180, maxColorValue = 255), df[, 'Color'])
